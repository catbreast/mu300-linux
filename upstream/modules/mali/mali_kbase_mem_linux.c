...
/* MU300: vma->vm_flags became read-only in 6.3; these are the accessors, with the plain assignment kept for
 * the vendor 5.4 kernel this driver is also built against. */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0)
#define mu300_vm_flags_set(vma, f) vm_flags_set((vma), (f))
#define mu300_vm_flags_clear(vma, f) vm_flags_clear((vma), (f))
#else
#define mu300_vm_flags_set(vma, f) ((vma)->vm_flags |= (f))
#define mu300_vm_flags_clear(vma, f) ((vma)->vm_flags &= ~(f))
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0)
	/* MU300: __GFP_ATOMIC is gone; "no direct reclaim allowed" is the same condition */
	WARN(!(sc->gfp_mask & __GFP_DIRECT_RECLAIM),
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 7, 0)
	/* MU300: shrinkers are allocated and registered separately now */
	kctx->reclaim = shrinker_alloc(0, "mali-kbase");
	if (!kctx->reclaim)
		return -ENOMEM;
		...
		unsigned long size, u64 *va_pages, u64 *flags)
{
	...
	*flags &= ~BASE_MEM_SAME_VA;
	...
	/* MU300: the vmas argument is gone since 6.5 */
	if (pages != NULL)
		faulted_pages = pin_user_pages(address, *va_pages, write ? FOLL_WRITE : 0, pages);
	else
		...
		int free_on_close)
{
	...
	mu300_vm_flags_set(vma, VM_DONTCOPY | VM_DONTDUMP | VM_DONTEXPAND | VM_IO);
	...
	if (!kaddr) {
		mu300_vm_flags_set(vma, VM_PFNMAP);
	} else {
		WARN_ON(aligned_offset);
		/* MIXEDMAP so we can vfree the kaddr early and not track it after map time */
		mu300_vm_flags_set(vma, VM_MIXEDMAP);
	...
	struct vm_area_struct *const vma)
{
	...
	if (!(vma->vm_flags & VM_READ))
		mu300_vm_flags_clear(vma, VM_MAYREAD);
	if (!(vma->vm_flags & VM_WRITE))
		mu300_vm_flags_clear(vma, VM_MAYWRITE);
...
static void kbasep_add_mm_counter(struct mm_struct *mm, int member, long value)
{
	...
	/* MU300: rss_stat became per-cpu counters. add_mm_counter() would do it, but it also fires the
	 * mm_trace_rss_stat tracepoint, which is not exported to modules - so update the counter directly.
	 */
...
static int kbase_tracking_page_setup(struct kbase_context *kctx, struct vm_area_struct *vma)
{
	...
	/* no real access */
	mu300_vm_flags_clear(vma, VM_READ | VM_MAYREAD | VM_WRITE | VM_MAYWRITE | VM_EXEC | VM_MAYEXEC);
	mu300_vm_flags_set(vma, VM_DONTCOPY | VM_DONTEXPAND | VM_DONTDUMP | VM_IO);
