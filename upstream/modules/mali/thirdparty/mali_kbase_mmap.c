...
/*
 * MU300: this function is the kernel's own unmapped_area_topdown() with extra alignment rules, and it walked
 * the VMA rbtree - which is gone since 6.1 (the maple tree replaced it). The kernel's vm_unmapped_area() does
 * the search now, so ask it for a gap and, if the gap does not satisfy the DDK's extra rules (shader code must
 * not touch a 4 GB boundary, some allocations must stay inside one 4 GB chunk), search again below the gap we
 * were given until one fits or the range runs out.
 */
static unsigned long kbase_unmapped_area_topdown(struct vm_unmapped_area_info *info, bool is_shader_code,
						 bool is_same_4gb_page)
{
	struct mm_struct *mm = current->mm;
	unsigned long end = info->high_limit;

	/* vm_unmapped_area() is not exported to modules, so walk the mappings downwards with find_vma(),
	 * which is. Each step looks at the gap that ends at "end" and, if the DDK's extra rules reject it,
	 * continues below the mapping that blocked it.
	 */
	while (end >= info->low_limit + info->length) {
		struct vm_area_struct *vma;
		unsigned long gap_start, gap_end = end;

		vma = find_vma(mm, end - info->length);
		if (vma && vma->vm_start < end) {
			/* something is in the way: look below it */
			if (vma->vm_start < info->length)
				break;
			end = vma->vm_start;
			continue;
		}
