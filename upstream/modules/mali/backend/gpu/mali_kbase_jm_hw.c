...
void kbase_jm_wait_for_zero_jobs(struct kbase_context *kctx)
{
	...
	if (kbase_prepare_to_reset_gpu(kbdev, RESET_FLAGS_HWC_UNRECOVERABLE_ERROR)) {
		dev_err(kbdev->dev,
			"Issuing GPU soft-reset because jobs failed to be killed (within %d ms) as part of context %px (tgid:%d) termination (e.g. process exit), context_nr_job:%d refcount:%d\n",
			ZAP_TIMEOUT, kctx, kctx->tgid, kctx->jctx.job_nr, kctx->refcount);
