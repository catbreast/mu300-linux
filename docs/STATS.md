# Repository & Community Statistics — `dikeckaan/mu300-linux`

> Automatically archived and updated. Last snapshot: **2026-09-20 15:44:33 UTC**.

## Summary Overview

- ⭐ **Stars:** 40
- 🍴 **Forks:** 12
- 👀 **Watchers:** 40
- ⚠️ **Open Issues:** 0
- 📥 **Total Release Asset Downloads:** 142 (97 OS/Kernel images)
- 👥 **Recorded Page Views:** 1,154 (approx. 487 unique visitors)
- 💻 **Recorded Git Clones:** 668 (approx. 291 unique cloners)

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=dikeckaan/mu300-linux&type=Date)](https://star-history.com/#dikeckaan/mu300-linux&Date)

## Traffic & Clones History (Daily)

| Date | Page Views | Unique Visitors | Git Clones | Unique Cloners |
|---|---|---|---|---|
| **2026-09-19** | 131 | 47 | 110 | 51 |
| **2026-09-18** | 347 | 105 | 196 | 87 |
| **2026-09-17** | 657 | 328 | 300 | 125 |
| **2026-09-16** | 19 | 7 | 62 | 28 |

## Top Referring Sites

| Referrer | Total Views | Unique Visitors |
|---|---|---|
| github.com | 57 | 32 |
| Google | 6 | 4 |
| github-com.translate.goog | 6 | 3 |
| coolapk.com | 3 | 2 |
| Bing | 2 | 2 |
| chatgpt.com | 1 | 1 |
| web.telegram.org | 1 | 1 |

## Release Downloads Breakdown

### v2026.09.21 (2026-09-18) — Subtotal: 79 downloads

| Asset | Size | Downloads |
|---|---|---|
| `mu300-kernel.tar.gz` | 22.7 MB | **22** |
| `mu300-openwrt-rootfs.tar.gz` | 44.2 MB | **19** |
| `mu300-ubuntu-rootfs.tar.gz` | 104.3 MB | **13** |
| `SHA256SUMS` | 273 B | **25** |

### v2026.09.20 (2026-09-18) — Subtotal: 36 downloads

| Asset | Size | Downloads |
|---|---|---|
| `mu300-kernel.tar.gz` | 22.7 MB | **11** |
| `mu300-openwrt-rootfs.tar.gz` | 44.2 MB | **9** |
| `mu300-ubuntu-rootfs.tar.gz` | 104.2 MB | **5** |
| `SHA256SUMS` | 273 B | **11** |

### v2026.09.19 (2026-09-17) — Subtotal: 18 downloads

| Asset | Size | Downloads |
|---|---|---|
| `mu300-kernel.tar.gz` | 22.7 MB | **5** |
| `mu300-openwrt-rootfs.tar.gz` | 44.2 MB | **5** |
| `mu300-ubuntu-rootfs.tar.gz` | 104.2 MB | **2** |
| `SHA256SUMS` | 273 B | **6** |

### v2026.09.18 (2026-09-17) — Subtotal: 0 downloads

| Asset | Size | Downloads |
|---|---|---|
| `mu300-kernel.tar.gz` | 22.7 MB | **0** |
| `mu300-openwrt-rootfs.tar.gz` | 44.2 MB | **0** |
| `mu300-ubuntu-rootfs.tar.gz` | 104.2 MB | **0** |
| `SHA256SUMS` | 273 B | **0** |

### v2026.09.17 (2026-09-17) — Subtotal: 9 downloads

| Asset | Size | Downloads |
|---|---|---|
| `mu300-kernel.tar.gz` | 22.7 MB | **3** |
| `mu300-openwrt-rootfs.tar.gz` | 44.2 MB | **1** |
| `mu300-ubuntu-rootfs.tar.gz` | 104.2 MB | **2** |
| `SHA256SUMS` | 273 B | **3** |

